...
class TaskAutomationService {
    ...
    async createAutomation() {
        ...
        const condition = await this.configureCondition(eventType.value);
        if (!condition)
            return; // User cancelled
     ...
     */
    async configureCondition(eventType) {
        switch (eventType) {
            case 'file-changed': {
    ...
    async configureAction() {
        ...
        switch (actionType.value) {
            ...
            case 'update-task-status': {
                ...
                const taskStatus = await vscode.window.showQuickPick(['To Do', 'In Progress', 'Completed'], { placeHolder: 'Select new status' });
                if (!taskStatus)
                    return undefined;
                return {
     ...
     */
    onFileChanged(uri) {
        const relevantRules = this.rules.filter(rule => rule.enabled &&
    ...
    async executeAction(action, _context = {}) {
        switch (action.type) {
            ...
            case 'update-task-status':
                // Here we would integrate with a task management system
                // For now, we'll just show a notification
                vscode.window.showInformationMessage(`Would update task ${action.taskId || 'selected task'} to status: ${action.taskStatus}`);
                break;
        }
    }
    /**
    ...
    checkScheduledRules() {
        ...
        const scheduledRules = this.rules.filter(rule => rule.enabled &&
     ...
     */
    loadRules() {
        const savedRules = this.context.globalState.get('taskAutomation.rules');
        if (savedRules) {
    ...
    async manageRule(rule) {
        ...
        switch (action.value) {
            case 'toggle':
                ...
                break;
            case 'delete':
                const confirmDelete = await vscode.window.showWarningMessage(`Are you sure you want to delete the rule "${rule.name}"?`, 'Yes', 'No');
