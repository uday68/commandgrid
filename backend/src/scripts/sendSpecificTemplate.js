...
/**
 * Function to send a specific email template
 * @param {string} templateName - Name of the template to send
 */
async function sendSpecificTemplate(templateName) {
  ...
  if (!testData[templateName]) {
    ...
    logger.info(`Available templates: ${Object.keys(testData).join(', ')}`);
    return false;
  }
...
sendSpecificTemplate(templateName)
  .then(success => {
