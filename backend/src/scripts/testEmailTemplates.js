...
/**
 * Send emails with specified time delay between them
 */
async function sendTestEmails() {
  const templates = Object.keys(testData);
  logger.info(`Starting to send ${templates.length} test emails to ${targetEmail}`);
  ...
  for (let i = 0; i < templates.length; i++) {
    ...
    try {
      ...
      if (i < templates.length - 1) {
        logger.info(`Waiting before sending next email...`);
        await new Promise(resolve => setTimeout(resolve, 2 * 60 * 1000)); // 2 minutes between emails
      }
    } catch (error) {
